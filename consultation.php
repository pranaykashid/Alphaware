...
if(isset($_POST)){// echo "<pre>"; print_r($_POST); exit;
            ...
            $query = "INSERT INTO freeconsultation(name,email,mno,project_desc) VALUES ('$name','$email','$mno','$project_desc')";

				if(mysqli_query($conn, $query))
				{
		  			mail_one($name,$email,$mno,$project_desc);
...
function mail_one($name,$email,$mno,$project_desc)
{
    $mail = new PHPMailer(true);
	$mail->isSMTP();
    ...
    $mail->isHTML(true);
    $mail->Subject = "Consultation Details"; // 'Message Received (Contact Page)';
    // $mail->MsgHTML($msg);
    $mail->Body = "<h1>Consultation Details: </h1><br> 
                  <h3>Name : ".ucfirst($name)."<br>Email Id: $email <br>Mob No.: $mno <br>Project Description : $project_desc  </h3>";
